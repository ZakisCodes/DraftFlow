...
let currentSubject = ''; // Stores the currently selected text for the subject box
...
const SubjectModule = {
  ...
  showSubjectBox(selectedText) {
    // Don't show subject box in edit mode
    if (this.isEditMode) return;

    if (!elements.subjectBox || !elements.subjectText) return;
    ...
    // Set a new timeout to hide the subject box automatically
  ...
  hideSubjectBox() {
    ...
    // Clear any pending show/hide timeouts for consistency
    if (subjectTimeout) {
      clearTimeout(subjectTimeout);
      subjectTimeout = null;
    }
    ...
    // Only clear selection in preview mode
    if (!this.isEditMode) {
      setTimeout(() => {
        try {
  ...
  handleIframeSelection() {
    // Don't handle selection in edit mode
    if (SubjectModule.isEditMode) return;
  ...
  handleIframeMessage(event) {
    // Don't handle messages in edit mode
    if (SubjectModule.isEditMode) return;
  ...
  setupIframeSelectionListeners() {
    ...
    iframe.addEventListener('load', () => {
      try {
        ...
        iframeDoc.removeEventListener('mouseup', this.handleIframeSelection);
        iframeDoc.removeEventListener('selectionchange', this.handleIframeSelection);

        // Add listeners to iframe document
        iframeDoc.addEventListener('mouseup', this.handleIframeSelection);
        iframeDoc.addEventListener('selectionchange', this.handleIframeSelection);
